...
impl Certificate {
    ...
    ///
    /// Unlike `subject_public_key()?.to_spki_der()` — which re-encodes the SPKI
    /// from the parsed key and so can differ for a non-canonically encoded
    /// certificate — this returns the original on-the-wire bytes. Hash these
    /// directly for public-key pinning (e.g. curl/OpenSSL
    /// `--pinnedpubkey sha256//…`, which pins `SHA-256(SPKI)`).
    pub fn spki_der(&self) -> Result<&[u8], Error> {
        let mut seq = self.tbs_after_algid()?;
        DistinguishedName::decode(&mut seq)?; // issuer
        Validity::decode(&mut seq)?; // validity
        DistinguishedName::decode(&mut seq)?; // subject
        Ok(seq.read_element()?) // full SubjectPublicKeyInfo TLV
    }
...
mod tests {
    ...
    /// SubjectPublicKeyInfo, and `SHA-256` of it equals the public-key pin that
    /// curl/OpenSSL compute (`--pinnedpubkey sha256//…`). Fixture cert + pin
    /// generated by OpenSSL 3.x.
    #[test]
...
-----END CERTIFICATE-----\n";
        ...
        //  openssl dgst -sha256` — the exact value curl pins.
        ...
        // It is a well-formed SPKI TLV (SEQUENCE) that re-parses to the key.
        assert_eq!(spki[0], 0x30, "spki_der starts with the SEQUENCE tag");
        assert!(cert.subject_public_key().is_ok());
        // The authoritative interop check: SHA-256 of the on-wire SPKI equals
        // the OpenSSL/curl public-key pin, proving byte-exact agreement.
        assert_eq!(
