...
pub unsafe extern "C" fn pc_ec_sign(
    ...
    let st = guard(|| {
        ...
        let sig = match curve {
            ...
            | CurveId::Secp192k1
            | CurveId::P192
            | CurveId::Secp224k1 => sk.sign::<Sha256>(m),
            CurveId::P256
            | CurveId::Secp256k1
...
pub unsafe extern "C" fn pc_ec_verify(
    ...
    guard(|| {
        ...
        let ok = match key.curve() {
            ...
            | CurveId::Secp192k1
            | CurveId::P192
            | CurveId::Secp224k1 => key.verify::<Sha256>(m, &parsed),
            CurveId::P256
            | CurveId::Secp256k1
            | CurveId::BrainpoolP256r1
            | CurveId::P224
...
/// An opaque Ed448 private key.
pub struct PcEd448Key(Ed448PrivateKey);

/// Generates an Ed448 key, or NULL on failure.
#[unsafe(no_mangle)]
...
/// Parses a PKCS#8 `PRIVATE KEY` PEM into an Ed448 key handle, or NULL.
///
/// # Safety
/// `pem` must be valid for `len` bytes.
#[unsafe(no_mangle)]
...
/// The empty context is used (pure Ed448, RFC 8032 §5.2).
///
/// # Safety
/// `key` valid; `msg` valid for `msg_len`; buffer rules for `out`.
#[unsafe(no_mangle)]
...
/// Frees an Ed448 key. NULL is ignored.
///
/// # Safety
/// `key` from [`pc_ed448_generate`]/[`pc_ed448_from_pem`], not freed twice.
#[unsafe(no_mangle)]
