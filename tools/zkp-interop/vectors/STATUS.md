...
| rangeproof | **No** | **byte-exact, all four operations** — `rangeproof.json`: 58 proofs reproduced byte-identically by `sign` (verify/info/rewind all match the oracle) + 20 rejections; parameter selection cross-checked over 7040 combinations; regenerate with `gen-rangeproof-vectors.c` |
